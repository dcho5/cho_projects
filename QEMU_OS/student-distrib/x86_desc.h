...
#define NUM_ENTRIES 1024 // number of entries in each paging data structure
...
typedef union pde {
  struct{
    unsigned char present:1; //                     [0]
    unsigned char read_and_write:1; //              [1]
    unsigned char supervisor:1; //                  [2]
    unsigned char write_through:1; //               [3]
    unsigned char cache_disabled:1; //              [4]
    unsigned char accessed:1; //                    [5]
    unsigned char reserved:1; //                    [6]
    unsigned char page_size:1; //                   [7]
    unsigned char global:1; //                      [8]
    unsigned char available:3; //                [11:9]
    unsigned long pb_address:20; //             [31:12]
  } __attribute__((packed));
  uint32_t bits; //                              [31:0]
} pde;
...
typedef union pte {
  struct{
    ...
    unsigned char cache_disabled:1; //              [4]
    unsigned char accessed:1; //                    [5]
    unsigned char dirty:1; //                       [6]
    unsigned char pat:1; //                         [7]
    unsigned char global:1; //                      [8]
    unsigned char available:3; //                [11:9]
    unsigned long pb_address:20; //             [31:12]
  } __attribute__((packed));
...
// page directory entries aligned on 4 kB boundaries
pde page_directory[NUM_ENTRIES] __attribute__((aligned(4096)));
// page table entries aligned on 4 kB boundaries
pte page_table[NUM_ENTRIES] __attribute__((aligned(4096)));
